      ...
      <el-col :span="18">
        <div>
          ...
          <el-select v-model="mainValue" placeholder="选择分类">
            <el-option v-for="(item, index) in classValues" :key="index" :label="index" :value="index" />
          </el-select>
        </div>
        <div v-for="(item, key) in subSelects" :key="key">
          <span class="nowarp">{{ key }}:</span>
          <el-select v-model="subattr[key]" placeholder="选择属性">
            <el-option v-for="(option, index) in item" :key="index" :label="option" :value="option" />
          </el-select>
        </div>
      </el-col>
